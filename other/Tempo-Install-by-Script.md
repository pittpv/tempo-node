...
## Файл .env-tempo: загрузка и переменные

Скрипт загружает конфигурацию в порядке: сначала `SCRIPT_DIR/.env-tempo` (каталог скрипта), затем `$TEMPO_HOME/.env-tempo` (значения из второго файла переопределяют первые). При установке ноды (опция 1 или 2) скрипт **создаёт** `.env-tempo` в `$TEMPO_HOME`, если файла ещё нет (на основе `.env.example`). **После установки** отредактируйте этот файл при необходимости.
...
5. Скрипт остановит контейнер, при необходимости обновит образ и перезапустит ноду. При настроенном Telegram придёт уведомление о завершении.
